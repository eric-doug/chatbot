...
export function stringTime(date) {
     // 返回指定日期的年份
        ...
        // 根据本地时间返回指定日期的月份（ 从 0 到 11），返回下标从0开始，所以要加1
        var m = date.getMonth() + 1;
        m = m < 10 ? "0" + m : m;
        // 返回月份中的某一天，使用本地时间。返回值是 1 ~ 31 之间的一个整数
        var d = date.getDate();
        d = d < 10 ? "0" + d : d;
        // 返回时间的小时，本地时间显示。返回值是 0 （午夜） 到 23 （晚上 11 点）之间的一个整数
        var h = date.getHours();
        h = h < 10 ? "0" + h : h;
        // 返回时间的分钟，以本地时间显示。返回值是 0 ~ 59 之间的一个整数
        var minute = date.getMinutes();
        minute = minute < 10 ? "0" + minute : minute;
        // 返回时间的秒，以本地时间显示。返回值是 0 ~ 59 之间的一个整数。
...
export function formatDateH5 (time) {
